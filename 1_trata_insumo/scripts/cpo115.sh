#!/bin/bash
# -------------------------------------------------------------------------- #
# cpo115.sh - Realiza conversao de registro LILACS para versao 1.7  #
# -------------------------------------------------------------------------- #
# 
#     Chamada : cpo115.sh  
#     Exemplo : ./cpo115.sh <FI>
#
# -------------------------------------------------------------------------- #
...
if [ "$#" -ne "3" ]; then
  ...
  echo "Exemplo: $0 bbo cpo002 cpo115"
  exit 0
fi
...
echo "115 Designacao Especifica do Material"
echo "Apaga n para colocar o pipe |"
$DIRISIS/mx $2 "proc=if v115='pipe' then 'd115' fi" create=$3_1 -all now tell=5000

echo "Problema do pipe I"
$DIRISIS/mx $3_1 "proc=if v115='|' then 'd115','<115 0>pipe</115>' fi" create=$3_2 -all now tell=5000

echo "Traz tabela de controle para join"
$DIRISIS/mx seq=$DIRTAB/tab_115.txt create=tab_115 -all now
$DIRISIS/mx tab_115 "fst=1 0 v1/" fullinv=tab_115 tell=1

echo "Join dos campos validos"
$DIRISIS/mx $3_2 "join=tab_115,975:1=s(mpu,v115,mpl)" "proc='d32001'" create=$3_3 -all now tell=5000

echo "Problema do pipe II"
$DIRISIS/mx $3_3 "proc=if v975='pipe' then 'd975','<975 0>|</975>' fi" create=$3_4 -all now tell=5000

echo "Gera Master"
$DIRISIS/mx $3_4 "proc='S'" create=$3 -all now tell=5000

echo "Gera ISO"
$DIRISIS/mx $3 "proc='d*',if p(v975) then |<2 0>|v2|</2>|,(|<975>|v975|</975>|) fi" iso=$DIRWORK/$1/$3.iso -all now tell=5000
echo
