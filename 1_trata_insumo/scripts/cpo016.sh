...
$DIRISIS/mx $3_2 "proc='d16',if p(v16) then (if v16^*.2='s.' then '<16>'v16*7'</16>' else '<16>'v16'</16>' fi),fi" create=$3_3 -all now

echo "Inclusao de Afiliacao s.af quando nao tem sub 1"
$DIRISIS/mx $3_2 "proc='d16'(if p(v16) then if a(v16^1) then '<16 0>'v16'^1s. af</16>' else '<16 0>'v16'</16>' fi,fi)" create=$3_4 -all now

$DIRISIS/mx $3_4 "pft=(if size(v16^p)>2 then v2[1]'|CPO016|'v16^p/ fi)" lw=0 -all now >$DIROUTS/$1/Rel_country_afil_$3.txt
