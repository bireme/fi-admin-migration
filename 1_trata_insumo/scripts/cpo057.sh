...
if [ "$#" -ne "3" ]; then
  ...
  echo "Exemplo: $0 bbo cpo002 cpo057"
  exit 0
fi
...
echo "057 Pais do Evento - Descricao"
cp $DIRTAB/gutf8ans.id .

$DIRISIS/id2i gutf8ans.id create=gutf8ans
$DIRISIS/mx seq=$DIRTAB/gpais57.seq gizmo=gutf8ans create=gpais57 -all now tell=1000

$DIRISIS/mx $2 "gizmo=gpais57,57" create=$3_1 -all now

echo "Cria Relatorios"
$DIRISIS/mx $3_1 "pft=if p(v57) and nocc(v57)>2 then v2'|CPO057|'v57/ fi" -all now lw=0 >$DIROUTS/$1/Rel_$3.txt

echo "Cria master"
$DIRISIS/mx $3_1 "proc='S'" create=$3 -all now

echo "Cria ISO"
$DIRISIS/mx $3 "proc='d*',if p(v57) then |<2 0>|v2|</2>|,(|<957>|v57|</957>|) fi" iso=$DIRWORK/$1/$3.iso -all now tell=10000
