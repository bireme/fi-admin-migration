#!/bin/bash
# -------------------------------------------------------------------------- #
# cpo018.sh - Realiza conversao de registro LILACS para versao 1.7  #
# -------------------------------------------------------------------------- #
# 
#     Chamada : cpo018.sh  
#     Exemplo : ./cpo018.sh <FI>
#
# -------------------------------------------------------------------------- #
...
if [ "$#" -ne "3" ]; then
  ...
  echo "Exemplo: $0 bbo cpo040 cpo018"
  exit 0
fi
...
echo "018 Titulo (Monografico) - Idioma do titulo da monografico"
$DIRISIS/mx $2 mfrl=8388608 fmtl=8388608 "proc=@$DIRTAB/v18_sub_i.prc" create=$3_1 -all now
$DIRISIS/mx $3_1 "proc=(if p(v1800) then  '<18 0>',v1800,|^i|v940,'</18>' fi)" create=$3_2 -all now

$DIRISIS/mx $3_2 "proc='d918',(|<918>|v18|</918>|)" create=$3_3 -all now

echo "Cria master e ISO"
$DIRISIS/mx $3_3 "proc='S'" create=$3 -all now 

$DIRISIS/mx $3 "proc='d*',if p(v918) then |<2 0>|v2|</2>|,(|<918>|v918|</918>|) fi" iso=$DIRWORK/$1/$3.iso -all now tell=10000
echo
