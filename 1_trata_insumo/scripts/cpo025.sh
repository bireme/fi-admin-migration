#!/bin/bash
# -------------------------------------------------------------------------- #
# cpo025.sh - Realiza conversao de registro LILACS para versao 1.7  #
# -------------------------------------------------------------------------- #
# 
#     Chamada : cpo025.sh  
#     Exemplo : ./cpo025.sh <FI>
#
# -------------------------------------------------------------------------- #
...
if [ "$#" -ne "3" ]; then
  ...
  echo "Exemplo: $0 bbo cpo040 cpo025"
  exit 0
fi
...
echo "025 Titulo (colecao) - Idioma do titulo da colecao"
$DIRISIS/mx $2 mfrl=8388608 fmtl=8388608 "proc=@$DIRTAB/v25_sub_i.prc" create=$3_1 -all now
$DIRISIS/mx $3_1 "proc=(if p(v2500) then  '<25 0>',v2500,|^i|v940,'</25>' fi)" create=$3_2 -all now

$DIRISIS/mx $3_2 "proc='d925',(|<925>|v25|</925>|)" create=$3_3 -all now

echo "Cria master e ISO"
$DIRISIS/mx $3_3 "proc='S'" create=$3 -all now 

$DIRISIS/mx $3 "proc='d*',if p(v925) then |<2 0>|v2|</2>|,(|<925>|v925|</925>|) fi" iso=$DIRWORK/$1/$3.iso -all now tell=10000
echo
